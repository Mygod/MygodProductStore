...
<asp:Content ID="Content2" ContentPlaceHolderID="Content" runat="server">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="nowrap">ID列表</td>
                <td>可选。你的 EdgeMod 将会在列表中的 EdgeMod 全部安装完毕后被安装。若安装链包含环，则该环不会被安装。</td>
            </tr>
            <tr>
                <td class="nowrap">InstallBefore</td>
                <td class="nowrap">ID列表</td>
                <td>可选。你的 EdgeMod 将会在列表中的任意一个 EdgeMod 安装前被安装。若安装链包含环，则该环不会被安装。</td>
            </tr>
            <tr>
                <td class="nowrap">EngineVersion</td>
                <td class="nowrap">版本闭区间</td>
    ...
    <div>样例：&lt;level filename="level309_winter" leaderboard_id="44875" name_sfx="1st_contact" /&gt;</div>
    ...
    <div>第三堆文件的后一个x是从0到关卡高度（<a href="Level.aspx">在XML的/Level/@Size中定义</a>）-1的一个索引，表示从下到上第x层中的方块。请注意这里定义的方块在游戏中会影响方块移动，但在PC版中这些玩意儿并不会在关卡中显示。换句话说它们是悬空的。如果你觉得很不爽，你可以用 EdgeTool 的修复悬浮功能。（请在完成关卡设计后再使用）</div>
    <div>你可以使用 Windows 自带的画图修改那些 png 文件。放大到 800% 然后把有方块的地方涂成非纯黑色（#000000）即可。</div>
    
    <h3>附：测试关卡</h3>
    <div>edgemod 格式很适合玩家安装，不过如果你要用它测试关卡，那就像是在地狱中一样麻烦！</div>
    <div>测试关卡时你没有必要做成一个 mod 再安装，你可以直接修改 levels/mapping.xml，在 bonus 元素的结尾手动添加一个 level 元素，填写相应属性，再将你的新关放到合适的位置，然后启动游戏即可。</div>
